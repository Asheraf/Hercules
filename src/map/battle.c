...
static int64 battle_calc_cardfix(int attack_type, struct block_list *src, struct block_list *target, int nk, int s_ele, int s_ele_, int64 damage, int cflag, int wflag)
{
	...
	switch(attack_type){
		case BF_MAGIC:
			...
			if (tsc != NULL && tsc->data[SC_CLIMAX_BLOOM] != NULL && s_ele == ELE_FIRE
			 && (nk & (NK_NO_CARDFIX_DEF | NK_NO_ELEFIX)) == 0)
				damage = apply_percentrate64(damage, 200, 100);
			break;
		case BF_WEAPON:
		...
		case BF_MAGIC:
			switch(skill_id){
				...
				case KR_THUNDERING_FOCUS_S:
					skillratio += -100 + 1420 + 70 * (skill_lv - 1) + 7 * st->int_;
					RE_LVL_DMOD(100);
					break;
				...
				case KR_THUNDERING_ORB_S:
					skillratio += -100 + 1750 + 100 * (skill_lv - 1) + 8 * st->int_;
					RE_LVL_DMOD(100);
					break;
				case KR_THUNDERING_CALL:
					skillratio += -100 + 5200 + 200 * (skill_lv - 1);
					if (sc != NULL && sc->data[SC_TRUTH_OF_WIND] != NULL) {
						skillratio += 8 * st->int_;
						RE_LVL_DMOD(100);
					}
					break;
				...
				**/
				...
				case EM_DIAMOND_STORM:
					...
					if (sc != NULL && sc->data[SC_SUMMON_ELEMENTAL_DILUVIO] != NULL)
						skillratio += 7500 - 300 * skill_lv + 5 * st->spl;
					RE_LVL_DMOD(100);
					break;
				case EM_LIGHTNING_LAND:
					skillratio += -100 + 700 + 1100 * skill_lv + 5 * st->spl;
					if (sc != NULL && sc->data[SC_SUMMON_ELEMENTAL_PROCELLA] != NULL)
						skillratio += 200 * skill_lv + 2 * st->spl;
					RE_LVL_DMOD(100);
					break;
				case EM_VENOM_SWAMP:
					skillratio += -100 + 700 + 1100 * skill_lv + 5 * st->spl;
					if (sc != NULL && sc->data[SC_SUMMON_ELEMENTAL_SERPENS] != NULL)
						skillratio += 200 * skill_lv + 2 * st->spl;
					RE_LVL_DMOD(100);
					break;
				...
				case SOA_TALISMAN_OF_SOUL_STEALING:
					...
					if (sd != NULL) {
						skillratio += pc->checkskill(sd, SOA_TALISMAN_MASTERY) * 7 * skill_lv;
						skillratio += pc->checkskill(sd, SOA_SOUL_MASTERY) * 7 * skill_lv;
					}
					skillratio += 3 * st->spl;
					RE_LVL_DMOD(100);
					break;
				...
				case HN_JUPITEL_THUNDER_STORM:
					...
					if (sd != NULL)
						skillratio += pc->checkskill(sd, HN_SELFSTUDY_SOCERY) * 3 * skill_lv;
					skillratio += 3 * st->spl;
					RE_LVL_DMOD(100);
					if (sd != NULL)
				...
				case AG_ASTRAL_STRIKE_ATK:
					...
					break;
				case CD_ARBITRIUM:
					skillratio += -100 + 1200 * skill_lv + 10 * st->spl;
					if (sd != NULL)
				...
				case CD_PNEUMATICUS_PROCELLA:
					skillratio += -100 + 150 + 2100 * skill_lv + 10 * st->spl;
					if (sd != NULL)
						skillratio += 3 * pc->checkskill(sd, CD_FIDUS_ANIMUS);
					if (tst->race == RC_UNDEAD || tst->race == RC_DEMON) {
						skillratio += 50 + 150 * skill_lv;
						if (sd != NULL)
							skillratio += 2 * pc->checkskill(sd, CD_FIDUS_ANIMUS);
					}
					RE_LVL_DMOD(100);
					break;
				case CD_FRAMEN:
					...
					if (sd != NULL)
						skillratio += 5 * skill_lv * pc->checkskill(sd, CD_FIDUS_ANIMUS);
					if (tst->race == RC_UNDEAD || tst->race == RC_DEMON)
						skillratio += 50 * skill_lv;
					RE_LVL_DMOD(100);
					break;
				...
				case AG_CRYSTAL_IMPACT:
					skillratio += -100 + 400 + 1400 * skill_lv + 5 * st->spl;
					RE_LVL_DMOD(100);
					break;
				...
				case HN_NAPALM_VULCAN_STRIKE:
					...
					if (sd != NULL)
						skillratio += 4 * skill_lv * pc->checkskill(sd, HN_SELFSTUDY_SOCERY);
					RE_LVL_DMOD(100);
					if (sd != NULL)
		...
		case BF_WEAPON:
			switch( skill_id )
			{
				...
				case SS_KUNAIKUSSETSU:
					...
					skillratio += 5 * st->pow;
					if (sd != NULL)
						skillratio += 10 * skill_lv * pc->checkskill(sd, SS_KUNAIKAITEN);
					RE_LVL_DMOD(100);
					break;
				case SS_KUNAIKAITEN:
					...
					skillratio += 5 * st->pow;
					if (sd != NULL)
						skillratio += 100 * skill_lv * pc->checkskill(sd, SS_KUNAIWAIKYOKU);
					RE_LVL_DMOD(100);
					break;
				case SS_KUNAIWAIKYOKU:
					...
					skillratio += 5 * st->pow;
					if (sd != NULL)
						skillratio += 10 * skill_lv * pc->checkskill(sd, SS_KUNAIKUSSETSU);
					RE_LVL_DMOD(100);
					if ((flag & SKILL_ALTDMG_FLAG) != 0)
						skillratio = skillratio * 3 / 10;
					break;
				...
				case SS_FUUMASHOUAKU:
					...
					skillratio += 5 * st->pow;
					if (sd != NULL)
						skillratio += 5 * skill_lv * pc->checkskill(sd, SS_FUUMAKOUCHIKU);
					RE_LVL_DMOD(100);
					break;
				case SS_KAGEGISSEN:
				...
				case SKE_SKY_MOON:
					...
					skillratio += 5 * st->pow;
					if (sd != NULL)
						skillratio += 9 * skill_lv * pc->checkskill(sd, SKE_SKY_MASTERY);
					RE_LVL_DMOD(100);
					break;
				case SKE_SKY_SUN:
				...
				case SKE_MIDNIGHT_KICK:
					...
					if (sd != NULL)
						skillratio += 5 * skill_lv * pc->checkskill(sd, SKE_SKY_MASTERY);
					if (sc != NULL && (sc->data[SC_MIDNIGHT_MOON] != NULL || sc->data[SC_SKY_ENCHANT] != NULL))
				...
				case SH_HYUN_ROK_CANNON:
					...
					if (sd != NULL) {
						...
						if (skill->sh_communed(src, SH_COMMUNE_WITH_HYUN_ROK) == true) {
							...
							skillratio += 25 * pc->checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY);
						}
					}
					RE_LVL_DMOD(100);
					break;
				...
				case NW_ONLY_ONE_BULLET:
					skillratio += -100 + 1200 + 3000 * skill_lv + 5 * st->con;
					if (sc != NULL && sc->data[SC_INTENSIVE_AIM_COUNT] != NULL)
				...
				case NW_GRENADES_DROPPING:
					skillratio += -100 + 550 + 850 * skill_lv + 5 * st->con;
					if (sd != NULL)
				...
				case BO_MAYHEMIC_THORNS:
					skillratio += -100 + 200 + 340 * skill_lv + 5 * st->pow;
					if (sc != NULL && sc->data[SC_RESEARCHREPORT] != NULL)
						skillratio += 200;
					RE_LVL_DMOD(100);
					break;
				...
				case SHC_CROSS_SLASH:
					skillratio += -100 + 300 * skill_lv + 5 * st->pow;
					if (sc != NULL && sc->data[SC_SHADOW_EXCEED] != NULL)
						skillratio += 60 * skill_lv + 2 * st->pow;
					RE_LVL_DMOD(100);
					break;
				...
				case HN_MEGA_SONIC_BLOW:
					...
					if (sd != NULL)
						skillratio += pc->checkskill(sd, HN_SELFSTUDY_TATICS) * 5 * skill_lv;
					skillratio += 5 * st->pow;
					if (status_get_hp(target) < status_get_max_hp(target) / 2)
						skillratio *= 2;
					RE_LVL_DMOD(100);
				...
				case KR_NASTY_SLASH:
					skillratio += -100 + 1150 + 50 * (skill_lv - 1) + 4 * st->str;
					if (sc != NULL && sc->data[SC_ENRAGE_WOLF] != NULL)
						skillratio += 300;
					RE_LVL_DMOD(100);
					break;
				case KR_DOUBLE_SLASH:
					skillratio += -100 + 1200 + 80 * (skill_lv - 1) + 6 * st->str;
					if (sc != NULL && sc->data[SC_ENRAGE_WOLF] != NULL)
						skillratio += 400;
					RE_LVL_DMOD(100);
					break;
				...
				case DR_NOMERCY_CLAW:
					...
					break;
				case KR_CLAW_WAVE:
					skillratio += -100 + 880 + 70 * (skill_lv - 1) + 5 * st->str;
					if (sc != NULL && sc->data[SC_ENRAGE_WOLF] != NULL)
						skillratio += 320;
					RE_LVL_DMOD(100);
					break;
				...
				case AT_FRENZY_FANG:
					...
					if (sc != NULL && (sc->data[SC_ALPHA_PHASE] != NULL || sc->data[SC_INSANE] != NULL
				...
				case AT_FERAL_CLAW:
					...
					break;
				case AT_PRIMAL_CLAW:
					skillratio += -100 + 1250 + 1050 * (skill_lv - 1) + 5 * st->pow;
					if (sc != NULL && (sc->data[SC_ALPHA_PHASE] != NULL || sc->data[SC_INSANE] != NULL
						|| sc->data[SC_INSANE2] != NULL || sc->data[SC_INSANE3] != NULL))
						skillratio += 800;
					RE_LVL_DMOD(100);
				...
				case KR_SHARPEN_HAIL:
					skillratio += -100 + 640 + 40 * (skill_lv - 1) + 4 * st->dex;
					if (sc != NULL && sc->data[SC_ENRAGE_RAPTOR] != NULL)
						skillratio += 300;
					RE_LVL_DMOD(100);
					break;
				case KR_TYPHOON_WING:
					skillratio += -100 + 600 + 80 * (skill_lv - 1) + 4 * st->dex;
					if (sc != NULL && sc->data[SC_ENRAGE_RAPTOR] != NULL)
						skillratio += 300;
					RE_LVL_DMOD(100);
					break;
...
static struct Damage battle_calc_magic_attack(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int mflag)
{
	...
	if( skill_id == SO_PSYCHIC_WAVE ) {
		if( sc && sc->count ) {
			...
			else if (sc->data[SC_DEEP_POISONING_OPTION] != NULL) s_ele = sc->data[SC_DEEP_POISONING_OPTION]->val3;
		}
	}
...
#define MATK_ADD( a ) ( ad.damage+= (a) )
				...
				switch(skill_id){
					...
					case MG_LIGHTNINGBOLT:
						...
						if ((skill_id == MG_FIREBOLT && sc != NULL && sc->data[SC_FLAMETECHNIC_OPTION] != NULL)
						 || (skill_id == MG_COLDBOLT && sc != NULL && sc->data[SC_COLD_FORCE_OPTION] != NULL)
						 || (skill_id == MG_LIGHTNINGBOLT && sc != NULL && sc->data[SC_GRACE_BREEZE_OPTION] != NULL))
							skillratio *= 5;
					/* Fall through */
					default:
		...
		} else if (skill_id == AG_ALL_BLOOM_ATK && sc != NULL && sc->data[SC_CLIMAX] != NULL
		        && sc->data[SC_CLIMAX]->val1 == 3) {
			skill_damage_bonus += 300;
		}
		if (sc != NULL && sc->data[SC_CLIMAX] != NULL) {
			if (skill_id == AG_CRYSTAL_IMPACT) {
				if (sc->data[SC_CLIMAX]->val1 == 3)
					skill_damage_bonus += 50;
				else if (sc->data[SC_CLIMAX]->val1 == 4)
					skill_damage_bonus -= 50;
			} else if (skill_id == AG_CRYSTAL_IMPACT_ATK && sc->data[SC_CLIMAX]->val1 == 4)
				skill_damage_bonus += 150;
		}
...
static struct Damage battle_calc_weapon_attack(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int wflag)
{
	...
	sd = BL_CAST(BL_PC, src);
	tsd = BL_CAST(BL_PC, target);
	if (skill_id == NW_ONLY_ONE_BULLET && sd != NULL && sd->weapontype1 == W_REVOLVER)
	...
	if(skill_id) {
		...
		switch(skill_id) {
			...
			case AT_FRENZY_FANG:
				if (sc != NULL) {
					if (sc->data[SC_ALPHA_PHASE] != NULL || sc->data[SC_INSANE3] != NULL)
						wd.div_ = 7;
		...
		if( skill_id ){
			switch(skill_id){
				case BO_ACIDIFIED_ZONE_WATER_ATK:
					...
					break;
				case BO_ACIDIFIED_ZONE_GROUND_ATK:
					rskill = BO_ACIDIFIED_ZONE_GROUND;
					break;
				case BO_ACIDIFIED_ZONE_WIND_ATK:
...
static enum damage_lv battle_weapon_attack(struct block_list *src, struct block_list *target, int64 tick, int flag)
{
	...
	if (sd) {
		...
		if (sc != NULL && sc->data[SC_COLD_FORCE_OPTION] != NULL && rnd()%100 < 7) {
			uint16 skill_lv = max(1, pc->checkskill(sd, MG_COLDBOLT));
			enum autocast_type ac_type = sd->auto_cast_current.type;
			int delay = skill->delay_fix(src, MG_COLDBOLT, skill_lv);

			sd->auto_cast_current.type = AUTOCAST_TEMP;
			if (status->charge(src, 0, skill->get_sp(MG_COLDBOLT, skill_lv)) != 0) {
				skill->castend_damage_id(src, target, MG_COLDBOLT, skill_lv, tick, flag);
				if (DIFF_TICK(sd->ud.canact_tick, tick + delay) < 0) {
					sd->ud.canact_tick = max(tick + delay, sd->ud.canact_tick);
					if (battle_config.display_status_timers != 0)
						clif->status_change(src, status->get_sc_icon(SC_POSTDELAY),
		...
		if (sc != NULL && sc->data[SC_GRACE_BREEZE_OPTION] != NULL && rnd()%100 < 7) {
			uint16 skill_lv = max(1, pc->checkskill(sd, MG_LIGHTNINGBOLT));
			enum autocast_type ac_type = sd->auto_cast_current.type;
			int delay = skill->delay_fix(src, MG_LIGHTNINGBOLT, skill_lv);
		...
		if (sc != NULL && sc->data[SC_DEEP_POISONING_OPTION] != NULL && rnd()%100 < 7) {
			uint16 skill_lv = max(1, pc->checkskill(sd, SO_POISON_BUSTER));
			enum autocast_type ac_type = sd->auto_cast_current.type;
			int delay = skill->delay_fix(src, SO_POISON_BUSTER, skill_lv);

			sd->auto_cast_current.type = AUTOCAST_TEMP;
			if (status->charge(src, 0, skill->get_sp(SO_POISON_BUSTER, skill_lv)) != 0) {
				skill->castend_damage_id(src, target, SO_POISON_BUSTER, skill_lv, tick, flag);
				if (DIFF_TICK(sd->ud.canact_tick, tick + delay) < 0) {
					sd->ud.canact_tick = max(tick + delay, sd->ud.canact_tick);
					if (battle_config.display_status_timers != 0)
						clif->status_change(src, status->get_sc_icon(SC_POSTDELAY),
...
static const struct config_data_old battle_data[] = {
	...
	{ "max_ap",                             &battle_config.max_ap,                          1000,   0,      INT_MAX,        },
	{ "max_cart_weight",                    &battle_config.max_cart_weight,                 8000,   100,    1000000,        },
	{ "max_parameter",                      &battle_config.max_parameter,                   99,     10,     10000,          },
	{ "max_trait_parameter",                &battle_config.max_trait_parameter,             110,    10,     10000,          },
...
// eAthena additions
	...
	{ "trait_points_job_change",            &battle_config.trait_points_job_change,         7,      0,      1000,           },
